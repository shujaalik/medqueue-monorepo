              ...
              <>
                <div className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center text-white shadow-lg flex-shrink-0 ${
                  queue.currentServing.isEmergency
                    ? queue.currentServing.severityScore >= 8
                      ? "bg-gradient-to-br from-red-600 to-rose-950 border border-red-500 shadow-red-500/20 animate-pulse"
                      : queue.currentServing.severityScore >= 5
                        ? "bg-gradient-to-br from-orange-500 to-red-600 border border-orange-400 shadow-orange-500/15 animate-pulse"
                        : "bg-gradient-to-br from-amber-400 to-orange-500 border border-amber-300 shadow-amber-500/10"
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    ...
                    {queue.currentServing.isEmergency && (
                      <span className={`text-[8px] font-black px-2 py-0.5 rounded-md animate-pulse uppercase tracking-wider text-white ${
                        queue.currentServing.severityScore >= 8
                        ...
                        Urgent Case (L{queue.currentServing.severityScore || 5})
                      </span>
                    )}
                  </div>
                      ...
                      {queue.activeTokens.slice(0, 3).map((p, idx) => (
                        <div key={idx} className="flex items-center justify-between text-xs font-semibold">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-slate-400 font-medium">ETA: {p.expectedTime || '--:--'}</span>
                          </div>
                          {p.isEmergency ? (
                            <span className={`text-[8px] font-black px-2 py-0.5 rounded-md text-white ${
                              ...
                              EMG (L{p.severityScore || 5})
                            </span>
                          ) : (
                            <span className="bg-emerald-50 text-emerald-700 text-[8px] font-black px-1.5 py-0.5 rounded-md">
                              Token {p.token}
                            </span>
                          )}
                        </div>
            ...
            <div className="bg-white rounded-3xl border border-emerald-100 p-5 flex flex-col">
              ...
              <div className="mt-3 flex justify-end">
                <button
                  ...
                  disabled={isExpanding || !shorthand.trim()}
                  className={`px-4.5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all flex items-center space-x-1.5 shadow-sm ${shorthand.trim() && !isExpanding
                ...
                >
                  ...
                  <span>{isExpanding ? 'AI Scribing...' : 'AI Expand Notes'}</span>
                </button>
              </div>
            </div>
                ...
                <div className="xl:col-span-2 bg-white rounded-[2rem] border border-emerald-100/60 shadow-xl p-6 flex flex-col h-[650px] relative">
                  ...
                  <form onSubmit={handleConsultAIScribe} className="pt-4 border-t border-emerald-50 flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className={`px-5 py-3.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all shadow-sm flex items-center justify-center gap-1.5 ${chatInput.trim() && !isConsulting
                        ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-600/10'
                        : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                        }`}
                    >
                      {isConsulting && <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
                      <span>{isConsulting ? 'Sending...' : 'Send'}</span>
                    </button>
                  </form>
                </div>
                    ...
                    <div className="bg-[#fcfdfd] border border-emerald-100/40 rounded-2xl p-4 shadow-sm space-y-4">
                      ...
                      <div className="space-y-2">
                        {expandedNotes.advice.map((adv, idx) => (
                          <div key={idx} className="bg-white border border-slate-100 rounded-xl p-3 flex items-center justify-between shadow-sm hover:border-slate-300 transition-all">
                            <span className="text-slate-400 mr-2 text-xs flex-shrink-0">»</span>
                            <input
                              type="text"
                              value={adv}
                              onChange={(e) => updateAdvice(idx, e.target.value)}
                              className="flex-1 bg-transparent text-slate-600 font-bold text-xs italic outline-none focus:ring-1 focus:ring-slate-100 rounded px-1"
                            />
                            <button
                              type="button"
                        ...
                        {(expandedNotes.questions || []).map((q, idx) => (
                          <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:border-amber-200 transition-all space-y-3">
                            ...
                            <div className="flex items-center space-x-2 pt-1 border-t border-slate-50">
                              ...
                              >
                                {isConsulting && <span className="w-2.5 h-2.5 border border-white border-t-transparent rounded-full animate-spin" />}
                                <span>{isConsulting ? 'Updating...' : 'Submit'}</span>
                              </button>
                            </div>
                          </div>
                        ))}
